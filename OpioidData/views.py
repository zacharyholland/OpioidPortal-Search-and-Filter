...
def searchPrescribersPageView(request) :
    if request.method == "POST" :
        ...
        prescribers = Prescriber.objects.filter(npi__icontains=npi).filter(Fname__icontains=name).filter(Lname__icontains=name).filter(Gender__icontains=gender).filter(isopioid_prescriber__icontains=opioid).filter(specialty__icontains=specialty).filter(State__icontains=state)
